...
run_rs(int m, int n, int k, int tp, int nnodes) {
  ...
  using ElementScale = decltype(to_cutlass_element(dt_conf.blockscale()));
  auto rt_conf = make_runtime_config(m, n, k, make_reduce_scatter_runtime_config(tp, nnodes));

  cutlass::DeviceAllocation<ElementA> block_A[kMaxTp];
  cutlass::DeviceAllocation<ElementB> block_B[kMaxTp];
  cutlass::DeviceAllocation<ElementD> block_D[kMaxTp];
